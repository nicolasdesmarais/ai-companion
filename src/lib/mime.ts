...
// keyword can appear anywhere in the file name
const sourceCodeKeywords = ["dockerfile", "jenkinsfile", "makefile"];

export function isSourceCodeFile(fileName: string) {
  fileName = fileName.toLocaleLowerCase();
  if (sourceCodeKeywords.some((t) => fileName.includes(t))) {
    return true;
  }
  const ext = extname(fileName).slice(1);
  return codeFileExtensions[ext] !== undefined;
}

export const getMime = (filename: string): string | false => {
  let mimeType = mime.lookup(filename);
  if (mimeType && sourceCodeMimes.includes(mimeType)) {
    return "text/plain";
  }
  if (!mimeType && isSourceCodeFile(filename)) {
    mimeType = "text/plain";
  }
  return mimeType;
};
...
export const codeFileExtensions = {
  ...
  po: "Gettext Catalog",
  // pot: "Gettext Catalog", // conflict with powerpoint
  glf: "Glyph",
  gp: "Gnuplot",
  gnu: "Gnuplot",
  gnuplot: "Gnuplot",
  ...
  graphql: "GraphQL",
  // dot: "Graphviz (DOT)", // conflict with word
  gv: "Graphviz (DOT)",
  man: "Groff",
  "1in": "Groff",
  "1m": "Groff",
...
// source code files that resolve to a mime type (but should be treated as text)
const sourceCodeMimes = [
